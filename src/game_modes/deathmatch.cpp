...
void applyDeathmatchHooks(void)
{
	DetourRemove(orig_setDMMode);
	orig_setDMMode = (setDMMode_type) DetourCreate((void*)0x5006D000,(void*)&my_setDMMode,DETOUR_TYPE_JMP, 5);

	*(unsigned int*)(&realrules) = 0x50128EB8;
}

/*
This is called by player_die too .. but i nopped it temporarily
Note: This function is also defined in features/alternate_scoreboard/alternate_scoreboard.cpp
If alternate_scoreboard feature is enabled, that version takes precedence.
*/
#ifndef FEATURE_ALTERATE_SCOREBOARD
...
void __thiscall my_setDMMode(void * self,int dmmode)
{
	...
	if ( sofree_dmmode > 0 ) {
		...
		int tmep_fix = 1;
		currentGameMode = game_modes[tmep_fix-1];

		// entry point
		stset(self,8,&dm_always);// rules
		...
		if ( dmmode == 1 || dmmode == 5 || dmmode == 4){
			...
			//set output funcs proxy pointer class
			currentGameMode->setOrigPointer((void*)(intercept));
			
		}
		else {
			// entrypoint is normal
