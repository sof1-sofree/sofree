...
void sf_sv_configstring_set(void)
{
	...
	// index string
	orig_PF_Configstring(index,orig_Cmd_Argv(2));
}
...
void sf_sv_save_reso(void)
{
	...
	*/
	...
	reso2d[slot][0] = atoi(orig_Cmd_Argv(2));
	reso2d[slot][1] = atoi(orig_Cmd_Argv(3));
}
...
void sf_sv_print_cprintf(void)
{
	...
	orig_cprintf(ent,PRINT_HIGH,orig_Cmd_Argv(2));
}

void sf_sv_print_bprintf(void)
{
	orig_bprintf(PRINT_HIGH,orig_Cmd_Argv(1));
}

void sf_sv_print_centerprint(void)
{
	edict_t * ent = get_ent_from_player_slot(atoi(orig_Cmd_Argv(1)));
	orig_centerprintf(ent,orig_Cmd_Argv(2));
}
...
{
	edict_t * ent = get_ent_from_player_slot(atoi(orig_Cmd_Argv(1)));
	orig_cinprintf(ent,atoi(orig_Cmd_Argv(2)),atoi(orig_Cmd_Argv(3)),atoi(orig_Cmd_Argv(4)),orig_Cmd_Argv(5));
}
...
void sf_sv_vector_set(void) {
	...
	vec3_t storethis;
	storethis[0] = atof(orig_Cmd_Argv(2));
	storethis[1] = atof(orig_Cmd_Argv(3));
	storethis[2] = atof(orig_Cmd_Argv(4));
...
void sf_sv_vector_copy(void) {
	...
	readCvarAsVector(copyname,out_vect);

	// read the VECT and write to CVAR
	writeCvarAsVector(out_vect,outname);
...
void sf_sv_player_anim(void)
{
	...
	if ( clientinst ) {
		...
		if ( !seq) {
	 	...
	 	GhoulPlay(seq,*(float*)0x5015CCDC,startPos,restart,loop,resume,reverse);
	}
}
...
void sf_sv_player_pos(void)
{
	...
	writeCvarAsVector(ent->s.origin,storecvar);
...
void sf_sv_ent_callback(void)
{
	...
	edict_t * ent = atoi(orig_Cmd_Argv(1));
	char *a2 = orig_Cmd_Argv(2);
	char *f = orig_Cmd_Argv(3);
	if ( !strcmp(a2,"think") ) {
