...
public:
	...
	virtual GhoulID FindMaterial(const char *Mat)=0; // 40
	virtual GhoulID NumMaterials() const=0; // 44
	virtual void GetMaterialName(GhoulID who,char *dest) const=0; // 48
	virtual GhoulID RegisterSkin(GhoulID Mat,const char *Skin,TexChannel Channel=Diffuse)=0; // 4c
	virtual GhoulID FindSkin(GhoulID Mat,const char *Skin,TexChannel Channel=Diffuse)=0; // 50
...
public:
	...
	virtual void ServerUpdate(float Time)=0;//B4
	virtual void TurnCallBacksOff()=0;//B8
	virtual void TurnMatrixCallBacksOff()=0;//BC
	virtual void TurnCallBacksOn(float Now)=0;//C0
	virtual void TurnMatrixCallBacksOn(float Now)=0;//C4
	virtual void GetBoltMatrix(float Time,Matrix4 &m,GhoulID Part=0,MatrixType kind=JacobianLocal,bool ToRoot=true)=0;//C8
	virtual void GetBoundBox(float Time,const Matrix4 &ToWorld,Vect3 &mins,Vect3 &maxs,GhoulID Part=0,bool Accurate=false)=0;//CC
	virtual int QueueForClient(float dist)=0;//D0
	virtual void Destroy()=0;//D4

	virtual GhoulID GetStateSequence()=0;//D8
// string based interface

	bool IsLive() {return (GetPlayingSequence()!=0)&&GetOnOff();}//DC

	bool SetPartOnOff(const char *Part,bool OnOff)//E0
	{
	...
	bool GetPartOnOff(const char *Part)//E4
	{
		GhoulID t=GetGhoulObject()->FindPart(Part);
		if (!t)
			return false;
	...
	bool SetFrameOverride(const char *Mat,const char *Skin,const char *Part)//E8
	{
		GhoulID t=GetGhoulObject()->FindPart(Part);
	...
	bool SetFrameOverride(const char *Mat,const char *Skin,GhoulID Part=0)//EC
	{
		GhoulID m=GetGhoulObject()->FindMaterial(Mat);
		if (!m)
			return false;
	...
	bool SetFrameOverride(GhoulID Mat,const char *Skin,GhoulID Part=0)//F0
	{
		if (!Mat)
			return false;
	...
	bool SetFrameOverride(const char *Mat,GhoulID Skin,GhoulID Part=0)//F4
	{
		GhoulID m=GetGhoulObject()->FindMaterial(Mat);
		if (!m)
			return false;
	...
	GhoulID GetFrameOverride(const char *Mat,const char *Part,TexChannel Channel=Diffuse)//F8
	{
		GhoulID m=GetGhoulObject()->FindMaterial(Mat);
	...
	GhoulID GetFrameOverride(const char *Mat,GhoulID Part=0,TexChannel Channel=Diffuse)//FC
	{
		GhoulID m=GetGhoulObject()->FindMaterial(Mat);
		if (!m)
			return 0;
	...
	bool ClearFrameOverride(const char *Mat,const char *Part,TexChannel Channel)//100
	{
		GhoulID m=GetGhoulObject()->FindMaterial(Mat);
		GhoulID p=GetGhoulObject()->FindPart(Part);
		if (!m)
	...
	bool AddNoteCallBack(IGhoulCallBack *c,const char *Token)//108
	{
		GhoulID t=GetGhoulObject()->FindNoteToken(Token);
		AddNoteCallBack(c,t);
		return true;
	}
	bool RemoveNoteCallBack(IGhoulCallBack *c,const char *Token)//10C
	{
		GhoulID t=GetGhoulObject()->FindNoteToken(Token);
		RemoveNoteCallBack(c,t);
		return true;
	}
	bool AddMatrixCallBack(IGhoulCallBack *c,const char *Part,MatrixType kind)//110
	{
		GhoulID t=GetGhoulObject()->FindPart(Part);
		AddMatrixCallBack(c,t,kind);
		return true;
	}
	bool RemoveMatrixCallBack(IGhoulCallBack *c,const char *Part,MatrixType kind)//114
	{
		GhoulID t=GetGhoulObject()->FindPart(Part);
		RemoveMatrixCallBack(c,t,kind);
		return true;
	}
	float GetSequenceLength(const char *Seq,EndCondition ec)//118
	{
		GhoulID s=GetGhoulObject()->FindSequence(Seq);
		if (!s)
			return 0.0f;
	...
	bool Bolt(const char *bolt, IGhoulInst *Bolted, const char *BoltedBolt)//11C
	{
...
public:
	...
	virtual void SetPolyOffset(float factor,float unit)=0;//9C

	virtual int GetNumTexturesBound()=0;//A0
	virtual int GetNumTexturesBoundBytes()=0;//A4
	virtual int GetNumMeshesRendered()=0;//A8
	virtual int GetNumTrianglesRendered()=0;//AC
	virtual int GetNumTrianglesRenderedSpec()=0;//B0
	virtual int GetNumVertsRendered()=0;//B4
	virtual int GetNumCornersRendered()=0;//B8
	virtual int GetNumLightRaysRendered()=0;//BC
	virtual void ClearStats()=0;//C0
};
